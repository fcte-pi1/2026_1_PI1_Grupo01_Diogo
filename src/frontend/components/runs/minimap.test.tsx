...
describe("Minimapa", () => {
  ...
  it("deve posicionar o ícone do robô na posição da telemetria mais recente", () => {
    ...
    const grid = container.firstElementChild!;
    const celulaEsperada = grid.children[10];
    expect(celulaEsperada.querySelector("img")).not.toBeNull();
  });

  it("deve mapear y=0 para a última linha do grid", () => {
    const { container } = render(
      <Minimapa
        tamanho={4}
        telemetries={[
    ...
    const grid = container.firstElementChild!;
    const celulaInferiorEsquerda = grid.children[12];

    expect(celulaInferiorEsquerda.querySelector("img")).not.toBeNull();
  });
